	...
	*initClass {
	...
	test_playing {
		...
		// Test every and whenmod, which must be played
		{
			...
			var events = this.prPlayAndGetEvents(b,
				this.prPat("deg\"0 1 2 3\"").every(4->3, _.reverse(1)),
				expected.size, #[\degree, \dur], quant: 4);
			this.assertEquals(events, expected, "every - function");

			expected = [
				(degree: 0, dur: 0.5), // bar 0
				(degree: 2, dur: 0.25),
				(degree: 3, dur: 0.25),

				(degree: 0, dur: 0.5), // bar 1
				(degree: 2, dur: 0.25),
				(degree: 3, dur: 0.25),

				(degree: 0, dur: 0.5), // bar 2
				(degree: 2, dur: 0.25),
				(degree: 3, dur: 0.25),

				(degree: 3, dur: 0.25), // bar 3
				(degree: 2, dur: 0.25),
				(degree: 0, dur: 0.5),
				(degree: 3, dur: 0.25),
				(degree: 2, dur: 0.25),
				...
				(degree: 0, dur: 0.5), // bar 0 restarts
			];
